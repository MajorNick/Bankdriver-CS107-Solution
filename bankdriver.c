...
Worker(void *threadarg)
{
  ...
  while (1) {
    ...
    case ACTION_REPORT:
      ...
      pthread_mutex_lock(&bank->waitForWorkers);
      bank->finishedWorkers++;
      if (bank->finishedWorkers==bank->numWorkers){
       
        err = Report_DoReport(bank, action.u.reportArg.workerNum);  
        bank->finishedWorkers = 0;

       
      }else{ 
     
        pthread_cond_wait(&bank->stopWorkers,&bank->waitForWorkers);
    
      }
      pthread_mutex_unlock(&bank->waitForWorkers);
      if (err != 0) {
